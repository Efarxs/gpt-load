...
import (
	...
	translatorcommon "gpt-load/internal/cliproxy/translator/common"
...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if bytes.Equal(rawJSON, []byte("[DONE]")) {
		return finalizeOpenAIToResponses(st, requestForNamespace)
	}

	root := gjson.ParseBytes(rawJSON)
	obj := root.Get("object")
	if obj.Exists() && obj.String() == "chat.completion" {
		return emitResponsesFromNonStreamChat(st, modelName, originalRequestRawJSON, requestRawJSON, rawJSON)
	}
...
func finalizeOpenAIToResponses(st *oaiToResponsesState, requestForNamespace []byte) [][]byte {
	if st == nil || st.CompletedEmitted {
		return [][]byte{}
	}
	if !st.Started && st.ResponseID == "" {
...
func emitResponsesFromNonStreamChat(st *oaiToResponsesState, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte) [][]byte {
	if st.CompletedEmitted {
		return [][]byte{}
	}
	full := ConvertOpenAIChatCompletionsResponseToOpenAIResponsesNonStream(context.Background(), modelName, originalRequestRawJSON, requestRawJSON, rawJSON, nil)
	if len(full) == 0 {
		return [][]byte{}
	}
	nextSeq := func() int { st.Seq++; return st.Seq }
	if !st.Started {
		st.ResponseID = gjson.GetBytes(full, "id").String()
		st.Created = gjson.GetBytes(full, "created_at").Int()
		if st.ResponseID == "" {
			st.ResponseID = gjson.GetBytes(rawJSON, "id").String()
		}
		if st.Created == 0 {
			st.Created = gjson.GetBytes(rawJSON, "created").Int()
		}
		created := []byte(`{"type":"response.created","sequence_number":0,"response":{"id":"","object":"response","created_at":0,"status":"in_progress","background":false,"error":null,"output":[]}}`)
		created, _ = sjson.SetBytes(created, "sequence_number", nextSeq())
		created, _ = sjson.SetBytes(created, "response.id", st.ResponseID)
		created, _ = sjson.SetBytes(created, "response.created_at", st.Created)
		st.Started = true
		completed := []byte(`{"type":"response.completed","sequence_number":0}`)
		completed, _ = sjson.SetBytes(completed, "sequence_number", nextSeq())
		completed, _ = sjson.SetRawBytes(completed, "response", full)
		st.CompletedEmitted = true
		return [][]byte{
			emitRespEvent("response.created", created),
			emitRespEvent("response.completed", completed),
		}
	}
	completed := []byte(`{"type":"response.completed","sequence_number":0}`)
	completed, _ = sjson.SetBytes(completed, "sequence_number", nextSeq())
	completed, _ = sjson.SetRawBytes(completed, "response", full)
	st.CompletedEmitted = true
	return [][]byte{emitRespEvent("response.completed", completed)}
}
