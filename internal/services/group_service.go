...
type GroupCreateParams struct {
	...
	ModelParamOverrides map[string]any
...
func (s *GroupService) CreateGroup(ctx context.Context, params GroupCreateParams) (*models.Group, error) {
	...
	if err := validateModelParamOverrides(params.ModelParamOverrides); err != nil {
		return nil, NewI18nError(app_errors.ErrValidation, "validation.invalid_json_format", map[string]any{"error": err.Error()})
	}
	...
	group := models.Group{
		...
		ModelParamOverrides: params.ModelParamOverrides,
...
func (s *GroupService) UpdateGroup(ctx context.Context, id uint, params GroupUpdateParams) (*models.Group, error) {
	...
	if params.ModelParamOverrides != nil {
		if err := validateModelParamOverrides(params.ModelParamOverrides); err != nil {
			return nil, NewI18nError(app_errors.ErrValidation, "validation.invalid_json_format", map[string]any{"error": err.Error()})
		}
		group.ModelParamOverrides = params.ModelParamOverrides
	}
...
func validateModelParamOverrides(overrides map[string]any) error {
	if len(overrides) == 0 {
		return nil
	}
	for model, raw := range overrides {
		...
		if _, ok := raw.(map[string]any); !ok {
			return fmt.Errorf("overrides for model %q must be an object", model)
		}
	}
	return nil
}
