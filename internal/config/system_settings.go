...
func validateStringSettingValue(key, val string) error {
	...
	if key == "session_affinity_ttl" {
		if err := validateSessionAffinityTTL(val); err != nil {
			return fmt.Errorf("invalid value for %s (%q): %w", key, val, err)
		}
	}
	return nil
...
func validateSessionAffinityTTL(val string) error {
	...
	if err != nil {
		return fmt.Errorf("must be a Go duration such as 1h or 30m")
	}
	if d <= 0 {
