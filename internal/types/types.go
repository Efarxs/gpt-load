...
type SystemSettings struct {
	...
	KeyValidationRequireContent  bool   `json:"key_validation_require_content" default:"true" name:"config.key_validation_require_content" category:"config.category.key" desc:"config.key_validation_require_content_desc"`

	// 路由：协议转换与渠道亲和性
	EnableProtocolRouting bool   `json:"enable_protocol_routing" default:"false" name:"config.enable_protocol_routing" category:"config.category.routing" desc:"config.enable_protocol_routing_desc"`
...
// SessionAffinityDuration 返回会话亲和 TTL，非法值回退为 1 小时。
func (s SystemSettings) SessionAffinityDuration() time.Duration {
	raw := strings.TrimSpace(s.SessionAffinityTTL)
	if raw == "" {
		return time.Hour
	}
