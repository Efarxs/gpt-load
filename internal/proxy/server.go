...
func (ps *ProxyServer) HandleProxy(c *gin.Context) {
	...
	finalBodyBytes, route, err := applyProtocolRouting(
		group,
		originalGroup.Name,
		c.Request.URL.Path,
	...
	if err != nil {
		...
		msg := err.Error()
		if ce, ok := err.(*translator.ConvertError); ok && ce.Status > 0 {
			status = ce.Status
			msg = ce.Message
		}
		response.Error(c, app_errors.NewAPIError(&app_errors.APIError{
			HTTPStatus: status,
			Code:       "PROTOCOL_CONVERT_FAILED",
			Message:    msg,
		}, msg))
		return
	}

	isStream := channelHandler.IsStreamRequest(c, bodyBytes) || channelHandler.IsStreamRequest(c, finalBodyBytes)
	if route != nil && route.stream {
		isStream = true
	}
	if route != nil && route.converted && detectJSONStream(finalBodyBytes) {
		isStream = true
	}
...
func (ps *ProxyServer) executeRequestWithRetry(
	...
	finalBodyBytes = applyOutboundBodyFixes(finalBodyBytes, group)

	// Update request body if it was modified by redirection or outbound fixes
	...
	if err != nil || shouldRetryByStatus {
		...
		// 亲和开启时 429 走时间冷却，不计入黑名单失败次数
		if aff != nil && aff.enabled && statusCode == http.StatusTooManyRequests {
			ps.keyProvider.MarkCooldown(group.ID, apiKey.ID, parseRetryAfter(resp.Header))
		} else {
	...
	if aff != nil && aff.enabled && aff.videoCreate && resp != nil {
		peek, readErr := io.ReadAll(resp.Body)
		if readErr == nil {
			resp.Body = io.NopCloser(bytes.NewReader(peek))
			if videoID := extractCreatedVideoID(peek); videoID != "" {
				ps.keyProvider.BindVideo(group.ID, apiKey.ID, videoID, aff.ttl)
			}
		}
	}
	...
	if shouldInterceptModelList(c.Request.URL.Path, c.Request.Method) {
		...
		if route != nil && route.converted {
			if err := ps.writeConvertedResponse(c, resp, route, isStream); err != nil {
				logrus.WithError(err).Error("Failed to convert upstream response")
				if !c.Writer.Written() {
...
func detectJSONStream(body []byte) bool {
	var probe struct {
