...
var MessagesEnUS = map[string]string{
	...
	"config.enable_protocol_routing_desc":        "When enabled, convert the client protocol to this group's channel protocol and convert the response back. Same-protocol requests (including openai Images/Videos) pass through. Paths: /v1/chat/completions and /v1beta/openai/ → Chat Completions; /v1/responses → Responses; /v1/messages → Messages; Gemini generateContent → Gemini. The conversion target is the group's channel type.",
	"config.enable_channel_affinity":             "Channel Affinity",
	"config.enable_channel_affinity_desc":        "When enabled, keep the same session on the same upstream key and upstream URL; rebind when the key is unavailable or cooling down after 429.",
	...
	"config.max_concurrency_per_key_desc":        "In-flight request cap per upstream key. 0 means unlimited. Keys at capacity are skipped; if every key is full the request fails with no available keys.",
