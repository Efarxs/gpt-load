...
var MessagesEnUS = map[string]string{
	...
	"config.key_validation_require_content":      "Require Real Completion",
	"config.key_validation_require_content_desc": "When enabled, key tests must receive a parseable non-stream completion (e.g. Chat choices), not just HTTP 2xx. Disable to keep the old status-only check.",
	"config.enable_protocol_routing":             "Protocol Conversion",
	"config.enable_protocol_routing_desc":        "When enabled, convert the client protocol to this group's upstream protocol and convert the response back. Disabled groups stay transparent.",
	"config.enable_channel_affinity":             "Channel Affinity",
	"config.enable_channel_affinity_desc":        "When enabled, keep the same session on the same upstream key and rebind when the key is unavailable or cooling down after 429.",
	"config.session_affinity_ttl":                "Session Affinity TTL",
	"config.session_affinity_ttl_desc":           "How long a session-to-key binding is kept, e.g. 1h or 30m. Only used when channel affinity is enabled.",
	"config.strip_orphan_tool_flags":             "Strip Orphan Tool Flags",
	"config.strip_orphan_tool_flags_desc":        "When the outbound request has no tools, drop tool_choice and parallel_tool_calls to avoid upstream 400. Enabled by default.",
