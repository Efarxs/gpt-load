...
var MessagesZhCN = map[string]string{
	...
	"config.key_validation_require_content":      "真实探测响应",
	"config.key_validation_require_content_desc": "开启后，测试密钥必须拿到可解析的非流式补全内容（例如 Chat 的 choices），不能只看 HTTP 2xx。关闭则仍以状态码 2xx 判定成功。",
	"config.enable_protocol_routing":             "协议转换",
	"config.enable_protocol_routing_desc":        "开启后，按客户端入口协议转换成该分组渠道的上游协议，并在返回时还原。关闭时保持透明转发。",
	"config.enable_channel_affinity":             "渠道亲和性",
	"config.enable_channel_affinity_desc":        "开启后，同一会话尽量使用同一把上游 Key；Key 不可用或 429 冷却时自动改绑。",
	"config.session_affinity_ttl":                "会话亲和 TTL",
	"config.session_affinity_ttl_desc":           "会话与 Key 绑定的保留时长，例如 1h、30m。仅在开启渠道亲和性时生效。",
	"config.strip_orphan_tool_flags":             "清理无工具旗标",
	"config.strip_orphan_tool_flags_desc":        "出站请求没有 tools 时，自动删除 tool_choice 和 parallel_tool_calls，避免上游 400。默认开启。",

	// Category labels
	"config.category.basic":   "基础参数",
	"config.category.request": "请求设置",
	"config.category.key":     "密钥配置",
	"config.category.routing": "路由",
