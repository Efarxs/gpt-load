...
var MessagesZhCN = map[string]string{
	...
	"config.enable_request_body_logging_desc": "是否在请求日志中记录完整的请求体，以及模型重定向和参数覆盖之后实际发给上游的请求体。启用此功能会增加内存以及存储空间的占用。",
	...
	"config.enable_protocol_routing_desc":        "开启后，按客户端入口协议转换成该分组渠道的上游协议，并在返回时还原。关闭时保持透明转发。同协议（含 openai 对生图/视频）恒等透传。入口路径：/v1/chat/completions 与 /v1beta/openai/ 为 Chat Completions，/v1/responses 为 Responses，/v1/messages 为 Messages，Gemini 原生 generateContent 为 Gemini。转换目标由渠道类型决定。",
	"config.enable_channel_affinity":             "渠道亲和性",
	"config.enable_channel_affinity_desc":        "开启后，同一会话尽量使用同一把上游 Key 和同一条上游；Key 不可用或 429 冷却时自动改绑。",
	"config.session_affinity_ttl":                "会话亲和 TTL",
	"config.session_affinity_ttl_desc":           "会话与 Key 绑定的保留时长，例如 1h、30m。仅在开启渠道亲和性时生效。",
	"config.max_concurrency_per_key":             "每 Key 并发上限",
	"config.max_concurrency_per_key_desc":        "同一把上游 Key 允许的在途请求数。0 表示不限制。达到上限后改选其他 Key，全部打满则返回无可用密钥。",
