...
var MessagesJaJP = map[string]string{
	...
	"config.enable_protocol_routing_desc":        "有効にすると、クライアントのプロトコルをこのグループのチャネルプロトコルへ変換し、応答を元の形式に戻します。同一プロトコル（openai の画像/動画を含む）は透過します。パス：/v1/chat/completions と /v1beta/openai/ は Chat Completions、/v1/responses は Responses、/v1/messages は Messages、Gemini の generateContent は Gemini。変換先はチャネルタイプです。",
	"config.enable_channel_affinity":             "チャネルアフィニティ",
	"config.enable_channel_affinity_desc":        "有効にすると、同一セッションをできるだけ同じ上流キーと上流 URL に固定し、キーが利用不可または 429 クールダウン中なら再バインドします。",
	...
	"config.max_concurrency_per_key":             "キーあたりの同時実行上限",
	"config.max_concurrency_per_key_desc":        "1 つの上流キーが同時に処理できるリクエスト数。0 は無制限。上限に達したキーはスキップし、全て満杯なら利用可能なキーなしで失敗します。",
