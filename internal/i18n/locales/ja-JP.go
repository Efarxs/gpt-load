...
var MessagesJaJP = map[string]string{
	...
	"config.key_validation_require_content":      "実応答を検証",
	"config.key_validation_require_content_desc": "有効時、キー検証は HTTP 2xx だけでなく、非ストリームの補完本文（例: Chat の choices）が解析できることを要求します。無効時は従来どおりステータスコードのみで判定します。",
	"config.enable_protocol_routing":             "プロトコル変換",
	"config.enable_protocol_routing_desc":        "有効にすると、クライアントのプロトコルをこのグループの上流プロトコルへ変換し、応答を元の形式に戻します。無効時は透過転送のままです。",
	"config.enable_channel_affinity":             "チャネルアフィニティ",
	"config.enable_channel_affinity_desc":        "有効にすると、同一セッションをできるだけ同じ上流キーに固定し、キーが利用不可または 429 クールダウン中なら再バインドします。",
	"config.session_affinity_ttl":                "セッションアフィニティ TTL",
	"config.session_affinity_ttl_desc":           "セッションとキーの紐付けを保持する時間。例：1h、30m。チャネルアフィニティ有効時のみ使用します。",
	"config.strip_orphan_tool_flags":             "孤立ツールフラグを削除",
	"config.strip_orphan_tool_flags_desc":        "出方向リクエストに tools がない場合、tool_choice と parallel_tool_calls を削除して上流 400 を避けます。デフォルト有効。",
