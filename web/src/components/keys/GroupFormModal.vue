...
const visibleConfigOptions = computed(() => {
  if (formData.group_type === "aggregate") {
    return configOptions.value.filter(opt => !routingConfigKeys.includes(opt.key));
  }
...
function loadGroupData() {
  ...
  const groupType = props.group.group_type || "standard";
  const configItems = Object.entries(props.group.config || {})
    .filter(([key]) => groupType !== "aggregate" || !routingConfigKeys.includes(key))
    .map(([key, value]) => {
      return {
        key,
        value,
  ...
  Object.assign(formData, {
    ...
    model_param_overrides: JSON.stringify(props.group.model_param_overrides || {}, null, 2),
...
async function handleSubmit() {
  ...
  try {
    ...
    if (formData.model_param_overrides) {
      try {
        modelParamOverrides = JSON.parse(formData.model_param_overrides);
        for (const [key, value] of Object.entries(modelParamOverrides)) {
          if (typeof key !== "string" || key.trim() === "" || (value !== null && typeof value !== "object")) {
            message.error(t("keys.modelParamOverridesInvalid"));
            return;
          }
        }
      } catch {
          ...
          <n-collapse>
            <n-collapse-item name="advanced">
              ...
              <div class="config-section">
                ...
                <div style="margin-top: 12px; padding-left: 120px">
                  <n-button
                    ...
                    dashed
                    style="width: 100%"
                    :disabled="formData.configItems.length >= visibleConfigOptions.length"
                  >
                    <template #icon>
                      <n-icon :component="Add" />
                    </template>
              ...
              <div class="config-section">
                ...
                <n-form-item path="model_param_overrides">
                  <template #label>
                    <div class="form-label-with-tooltip">
                      {{ t("keys.modelParamOverrides") }}
                      <n-tooltip trigger="hover" placement="top">
                        <template #trigger>
                          <n-icon :component="HelpCircleOutline" class="help-icon config-help" />
                        </template>
                        <span>{{ t("keys.modelParamOverridesTooltip") }}</span>
                      </n-tooltip>
                    </div>
                  </template>
                  <n-input
                    v-model:value="formData.model_param_overrides"
                    type="textarea"
                    placeholder='{"gpt-5.6-luna":{"reasoning_effort":"none"}}'
                    :rows="5"
                  />
                  <template #feedback>
                    <div style="font-size: 14px; color: #999">
                      {{ t("keys.modelParamOverridesDescription") }}
                    </div>
                  </template>
                </n-form-item>
              </div>
