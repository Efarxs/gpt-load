...
async function testKey(_key: KeyRow) {
  ...
  try {
    ...
    testResult.value = {
      valid: !!curValid.is_valid,
      duration: formatDuration(response.total_duration),
      statusCode: curValid.status_code || 0,
      error: curValid.error || "",
      body: formatProbeBody(curValid.response_body || ""),
    };
...
function formatProbeBody(raw: string): string {
  const text = (raw || "").trim();
  if (!text) {
    return "";
  }
...
async function copyProbeBody() {
  if (!testResult.value?.body) {
    return;
  }
  const success = await copy(testResult.value.body);
  if (success) {
    window.$message.success(t("keys.keyCopied"));
  } else {
    window.$message.error(t("keys.copyFailed"));
...
<template>
  ...
  <n-modal
    ...
    :close-on-esc="true"
    :auto-focus="false"
    class="key-test-result-modal"
    @update:show="testResultShow = $event"
  >
    <n-card
      class="key-test-result-card"
    ...
    >
      <template #header-extra>
        <n-button quaternary circle @click="testResultShow = false">
          <template #icon>
      ...
      <div v-if="testResult" class="test-result-content">
        <div class="test-result-meta">
          <div class="meta-item">
            <span class="meta-label">{{ t("keys.testResultStatus") }}</span>
            <n-tag :type="testResult.valid ? 'success' : 'error'" size="small">
              {{ testResult.valid ? t("keys.testResultPassed") : t("keys.testResultFailed") }}
            </n-tag>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t("keys.testResultHttpStatus") }}</span>
            <span class="meta-value">{{ testResult.statusCode || "-" }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t("keys.testResultDuration") }}</span>
            <span class="meta-value">{{ testResult.duration }}</span>
          </div>
        </div>
        <div v-if="testResult.error" class="test-result-error">
          <div class="meta-label">{{ t("keys.testResultError") }}</div>
          <pre class="error-text">{{ testResult.error }}</pre>
        </div>
        <div class="test-result-body">
          <div class="body-header">
            <span class="meta-label">{{ t("keys.testResultBody") }}</span>
            <n-button
              size="tiny"
              quaternary
              ...
              @click="copyProbeBody"
            >
              {{ t("keys.testResultCopyBody") }}
            </n-button>
          </div>
          <pre class="body-text">{{ testResult.body || t("keys.testResultEmptyBody") }}</pre>
        </div>
      </div>

      <template #footer>
        <div class="modal-actions">
          <n-button type="primary" @click="testResultShow = false">
            {{ t("keys.testResultClose") }}
          </n-button>
        </div>
      </template>
    </n-card>
...
.test-result-error .error-text,
.test-result-body .body-text {
  margin: 6px 0 0;
  padding: 10px 12px;
  max-height: 360px;
  overflow: auto;
